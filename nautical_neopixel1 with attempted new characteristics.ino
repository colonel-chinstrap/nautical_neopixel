...
#define QFLASH_DURATION 250

// quick flashing period (flash once a second)
#define QUICK_PERIOD 1000

// quick flash duration
#define VQFLASH_DURATION 125

// very quick flashing period (flash twice per second)
#define VQUICK_PERIOD 500

// long flash duration
#define LFLASH_DURATION 2000

// long flash period
#define LONG_PERIOD 4000
...
char* nav_leds[LED_COUNT] = {
  							  "Fl (6) W 15s",
                              "Iso  W 4s", /*Isophase light, 4 seconds on, 4 seconds off*/
                              "Mo (U) W 15s", /* Morse letter U every 15s */
                              "Q (6+LFl) W 15s", /*Quick flash 6 times plus long (2 second) flash every 15s*/
                              "Q (2) W 7.5s", /*Quick flash (half second) twice every 7.5s*/
                              "VQ (9) W 10s", /*Very quick flash (quarter second) 9 times every 10s*/
                              //"VQ (9) W 10s",
                              //"Fl R 6s",
                              //"Fl (2+1) R 10s",
                              //"Fl W 6s",
   ...
   // }
	...
	// occulting
      ...
      if (str[2] == '(' and isdigit(str[3])) {
        ...
        else if(str[4] == '+' and isdigit(str[5]) and str[6] == ')') {
          // composite group flashing
          group1 = atoi(&str[3]);
          group2 = atoi(&str[5]);
          // offset pointer
...
void flash(unsigned int count, int led_idx, uint32_t color, int group1, int group2, int period) {
  ...
  if ((count % FLASH_PERIOD) > FLASH_DURATION) {
    strip.setPixelColor(led_idx, OFF);
  } else {
    int subframe = count / FLASH_PERIOD;
    // determine groupset
...
// qflashing
void qflash(unsigned int count, int led_idx, uint32_t color, int group1, int group2, int period) {
  
  count = count % period;
  
  if ((count % QUICK_PERIOD) > QFLASH_DURATION) {
    strip.setPixelColor(led_idx, OFF);
  } else {
...
// vqflashing
void vqflash(unsigned int count, int led_idx, uint32_t color, int group1, int group2, int period) {
  
  count = count % period;
  
  if ((count % VQUICK_PERIOD) > VQFLASH_DURATION) {
    strip.setPixelColor(led_idx, OFF);
  } else {
    int subframe = count / VQUICK_PERIOD;
    // determine groupset
    // assume half the period is for the first grouping
    int group = group1;
    if ((count * 2) >= period) {
...
// long flashing
void lflash(unsigned int count, int led_idx, uint32_t color, int group1, int group2, int period) {
  
  count = count % period;
  strip.setPixelColor(led_idx, color);  

  int subframe = count / LONG_PERIOD;
  // determine groupset
  // assume half the period is for the first grouping
  int group = group1;
  if ((count * 2) >= period) {
    group = group2;
    count = count % (period / 2);
    subframe = count / LONG_PERIOD;
  }
  if (subframe < group) {
    if ((count % LONG_PERIOD) > LFLASH_DURATION) {
      strip.setPixelColor(led_idx, color);
    } else {
      strip.setPixelColor(led_idx, OFF);
    }
